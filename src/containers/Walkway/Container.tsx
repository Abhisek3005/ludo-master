...
class WalkwayBare extends React.PureComponent<IProps> {
  ...
  private renderCells = () => {
    ...
    const cellConfigurationsByPosition = cellConfigurations[position] || {};
    const cellConfigurationsForWalkway = Object.values(cellConfigurationsByPosition);
    ...
    cellConfigurationsForWalkway.forEach((cellConfiguration, index) => {
      ...
      const cellType = (cellConfigurations[position] && cellConfigurations[position][cellID] && cellConfigurations[position][cellID].cellType) || cellConfiguration.cellType;
      ...
      >
        {
          (cellConfiguration.coinIDs || []).filter((coinID) => coins[coinID]).map((coinID, index) => (
            <Coin
  ...
  private onCoinClicked = (coinID: ICoin['coinID'], position: WalkwayPosition, cellID: ICell['cellID']) => {
    ...
    if (this.props.isDieRollValid && coin.baseID === currentTurn && onlineSession.canControlBase(coin.baseID)) {
