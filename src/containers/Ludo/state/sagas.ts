...
function * performDisqualificationCheck(activeCoinID: ICoin['coinID'], walkwayPosition: WalkwayPosition, cellID: ICell['cellID']) {
  ...
  if (cellInWhichCoinLanded.cellType === CellType.NORMAL) {
    ...
    for (const coinID of cellInWhichCoinLanded.coinIDs) {
      const coin = coins[coinID];
      if (activeCoin.baseID !== coin.baseID && !onlineSession.sameTeam(activeCoin.baseID, coin.baseID)) {
...
function * homeCoinSaga(action: ReturnType<typeof homeCoin>) {
  ...
  if (hasWon) {
    ...
    if (onlineSession.isTeamsMode()) {
      const teammateBaseID = getTeammateBaseID(baseID);
      const teammateBase = bases[teammateBaseID];
      if (teammateBase && teammateBase.hasWon) {
        window.alert(`Team ${baseID === BaseID.BASE_1 || baseID === BaseID.BASE_3 ? 'A' : 'B'} wins!`);
      }
    }
  }
}

function getTeammateBaseID(baseID: BaseID) {
  switch (baseID) {
    case BaseID.BASE_1:
      return BaseID.BASE_3;
    case BaseID.BASE_3:
      return BaseID.BASE_1;
    case BaseID.BASE_2:
      return BaseID.BASE_4;
    case BaseID.BASE_4:
      return BaseID.BASE_2;
    default:
      return BaseID.BASE_3;
  }
}
